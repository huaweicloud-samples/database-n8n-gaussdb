...
import type {
	...
	QueryValues,
	QueryWithValues,
} from '../../../Postgres/v2/helpers/interfaces';
import {
...
} from '../../../Postgres/v2/helpers/utils';
...
export async function execute(
	...
	for (let i = 0; i < items.length; i++) {
		try {
			...
			// GaussDB O模式不支持 INSERT ... ON CONFLICT，改用 MERGE INTO。
			// MERGE 也不支持 RETURNING，故 upsert 不返回行数据。
			const itemKeys = Object.keys(item);
			const updateColumns = itemKeys.filter(
				(column) => !columnsToMatchOn.includes(column),
			);

			// USING 子查询：每个 item 值一个参数，别名 c1, c2, ... 按 itemKeys 顺序
			let valuesLength = values.length + 1;
			const usingParts: string[] = [];
			itemKeys.forEach((column, idx) => {
				usingParts.push(`$${valuesLength} AS c${idx + 1}`);
				values.push(item[column] as string);
				valuesLength = valuesLength + 1;
			});

			// 列名参数（$N:name），可被 ON / UPDATE / INSERT 多处引用
			const columnNamePlaceholders: Record<string, string> = {};
			itemKeys.forEach((column) => {
				columnNamePlaceholders[column] = `$${valuesLength}:name`;
				values.push(column);
				valuesLength = valuesLength + 1;
			});

			const usingSelect = usingParts.join(', ');

			const onConditions = columnsToMatchOn
				.map((mc) => {
					const idx = itemKeys.indexOf(mc);
					return `t.${columnNamePlaceholders[mc]} = s.c${idx + 1}`;
				})
				.join(' AND ');

			const updateSet = updateColumns
				.map((uc) => {
					const idx = itemKeys.indexOf(uc);
					return `${columnNamePlaceholders[uc]} = s.c${idx + 1}`;
				})
				.join(', ');

			const insertCols = itemKeys.map((k) => columnNamePlaceholders[k]).join(', ');
			const insertValues = itemKeys.map((_, idx) => `s.c${idx + 1}`).join(', ');

			let query = `MERGE INTO $1:name.$2:name t USING (SELECT ${usingSelect}) s ON (${onConditions})`;

			if (updateColumns.length > 0) {
				query += ` WHEN MATCHED THEN UPDATE SET ${updateSet}`;
			}

			query += ` WHEN NOT MATCHED THEN INSERT (${insertCols}) VALUES (${insertValues})`;
